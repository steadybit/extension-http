...
func (c *bandwidthChecker) emitWindowMetric() *action_kit_api.Metric {
	...
	if len(statusCounts) > 0 {
		...
		metricLabels["http_status"] = formatCounts(statusCounts)
		metricLabels["expected_http_status"] = strconv.FormatBool(allExpected)
	}
	...
	if len(transportErrors) > 0 {
		metricLabels["error"] = formatCounts(transportErrors)
	}
